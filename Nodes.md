...
- Стрелки также должны быть аккуратно привязаны к паре узлов каждая и иметь спец. значение по смыслу. Стрелки не должны "висеть" или быть прицепленными к контуру узла.
  ...
  - Спец. назначаются значения стрелкам через меню _Изменить_ > _Изменить значение_, и после нажатия на кнопку "_Принять_" стрелка становится специальной. При принудительном изменении свойств стрелки через разблокирование её можно вернуть в специльное состояние при помощи той же кнопки.
...
Для булевой развилки нужно использовать проверки, т.е. операции семейства _check_.  
Для развилки по Enum выражение должно вернуть значение типа Enum. 
- Интересно, что нельзя вернуть логическое (булево) значение из свойства; вместо этого нужно определить для этого Enum с парой элементов и использовать их на выходящих стрелках.
...
Ниже описанные рекомендации применимы к онлайн редактору (м.б. неприменимы к настольной Draw.io).
